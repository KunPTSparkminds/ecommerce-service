...
public class AuthServiceImpl implements AuthService {
    ...
    public LoginResponse login(LoginRequestDTO loginRequestDTO) {
        ...
        boolean isMatchesPassword = bCryptPasswordEncoder.matches(loginRequestDTO.getPassword(), user.getPassword());
        if (isMatchesPassword == false) {
            throw new EntityNotFoundException("Password not correct");
        }
    ...
    public UserResponseDTO getUserDetail(String token) {
        ...
        return UserResponseDTO.builder().id(user.getId()).email(username).name(user.getName()).role(user.getRole())
                .build();
    }
